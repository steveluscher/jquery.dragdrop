...
(function() {
  ...
  jQuery(function() {
    jQuery.droppable = (function(_super) {
      var getClosestDroppable;

      __extends(droppable, _super);

      droppable.prototype.defaults = {
      ...
      function droppable(element, options) {
        var selector,
          _this = this;
        this.options = options != null ? options : {};
        this.handleDraggableStop = __bind(this.handleDraggableStop, this);
        this.handleDraggableStart = __bind(this.handleDraggableStart, this);
        if (typeof (selector = this.options.accept) === 'string') {
          this.options.accept = function(metadata) {
            return metadata.draggable.is(selector);
          };
        }
        droppable.__super__.constructor.apply(this, arguments);
        if (jQuery.draggable == null) {
          throw new Error('[jQuery DragDrop – Droppable] Missing dependency jQuery Draggable');
        }
        this.$element = $(element);
        this.$element.addClass(this.getConfig().droppableClass);
        $(jQuery.draggable.prototype).on({
          start: this.handleDraggableStart
        });
        if (jQuery.draggable.draggableAloft != null) {
          this.handleDraggableStart(jQuery.draggable.latestEvent, jQuery.draggable.draggableAloft);
        }
        this;
      }

      droppable.prototype.setupElement = function() {
        var callbackName, config, _i, _len, _ref;
        config = this.getConfig();
        _ref = ['over', 'out', 'drop'];
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      ...
      droppable.prototype.setupMouseOverListener = function() {
        var _this = this;
        this.$element.on('mouseover', function(e) {
          var $closestDroppableToOrigin;
          if (!_this.dragStarted || _this.isDropTarget) {
            return;
          }
          $closestDroppableToOrigin = getClosestDroppable(e.target);
          if (_this.$element.is($closestDroppableToOrigin)) {
            return _this.handleOver(e);
          }
        });
        return this.mouseOverListenerSetupPerformed = true;
      };

      droppable.prototype.setupMouseOutListener = function() {
        var _this = this;
        this.$element.on('mouseout', function(e) {
          var $closestDroppableToNextElement, $closestDroppableToOrigin;
          if (!_this.dragStarted) {
            return;
          }
          $closestDroppableToOrigin = getClosestDroppable(e.target);
          $closestDroppableToNextElement = getClosestDroppable(e.relatedTarget);
          if (_this.$element.is($closestDroppableToOrigin) && !_this.$element.is($closestDroppableToNextElement)) {
            return _this.handleOut(e);
          }
        });
        return this.mouseOutListenerSetupPerformed = true;
      };

      droppable.prototype.handleDraggableStart = function(e, draggable) {
        var $closestDroppableUnderMouse, elementUnderMouse;
        if (!this.setupPerformed) {
          this.setupElement();
        }
        if (!this.mouseOverListenerSetupPerformed) {
        ...
        return $(jQuery.draggable.prototype).on({
          stop: this.handleDraggableStop
        });
      };

      droppable.prototype.handleDraggableStop = function(e, draggable) {
        if (this.isDropTarget) {
          this.$element.removeClass(this.getConfig().hoverClass);
          this.isDropTarget = false;
          this.handleDrop(e.originalEvent);
        }
        $(jQuery.draggable.prototype).off({
          stop: this.handleDraggableStop
        });
        return this.cleanUp();
      };

      droppable.prototype.handleOver = function(e) {
        var dropOverEvent, eventMetadata, _base;
        eventMetadata = this.getEventMetadata();
        if (!this.getConfig().accept(eventMetadata)) {
          return;
        }
        if (!this.mouseOutListenerSetupPerformed) {
          this.setupMouseOutListener();
        }
        this.$element.addClass(this.getConfig().hoverClass);
        this.isDropTarget = true;
        dropOverEvent = this.synthesizeEvent('dropover', e);
        if (typeof (_base = this.getConfig()).over === "function") {
          _base.over(dropOverEvent, eventMetadata);
        }
        return this.$element.trigger(dropOverEvent, eventMetadata);
      };

      droppable.prototype.handleOut = function(e) {
        var dropOutEvent, eventMetadata, _base;
        this.$element.removeClass(this.getConfig().hoverClass);
        this.isDropTarget = false;
        eventMetadata = this.getEventMetadata();
      ...
      droppable.prototype.handleDrop = function(e) {
        var dropEvent, eventMetadata, _base;
        eventMetadata = this.getEventMetadata();
        dropEvent = this.synthesizeEvent('drop', e);
        if (typeof (_base = this.getConfig()).drop === "function") {
          _base.drop(dropEvent, eventMetadata);
        }
        return this.$element.trigger(dropEvent, eventMetadata);
      };
      ...
      droppable.prototype.cleanUp = function() {
        ...
        return delete this.$helper;
      };

      getClosestDroppable = function(element) {
        var $putativeDroppable;
        $putativeDroppable = $(element);
        while (!(!$putativeDroppable.length || $putativeDroppable.data('droppable'))) {
          $putativeDroppable = $putativeDroppable.parent();
        }
        return $putativeDroppable;
      };

      return droppable;

    })(jQuery.dragdrop);
    return $.fn.droppable = function(options) {
      return this.each(function() {
        var plugin;
        if ($(this).data('droppable') == null) {
          plugin = new $.droppable(this, options);
          return $(this).data('droppable', plugin);
        }
      });
    };
  });
